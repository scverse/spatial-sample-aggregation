...
def test_compute_node_feature(adata, metric, expected_shape):
    ...
    # Check index matches adata obs index
    assert result.index.equals(adata.obs.index), "Index of result does not match adata.obs.index."

    # Check no NaNs in degree and mean_distance (shannon may have NaNs for isolated nodes)
    if metric in ["degree", "mean_distance"]:
        assert not result.isna().any(), f"NaN values found in {metric} computation."
